...
**Extraction is cached per document, not re-run per act.** The hackathon campaign topped the account up to 500,000 credits, so this is no longer a budget decision — it is a design one, and it stands on its own: an extraction is bound to an immutable document, and re-running it on every act would buy nothing that re-checking the hash does not already buy. It is safe because the document's hash is re-checked against DNS on *every* act — change one byte and both the record and the stored terms stop matching, and everything denies. It is nonetheless **the one cached input in the enforcement path**, and it is stated here rather than buried in a code comment.
